// 📦 router.js – Fully Modular Routes Extracted from server.js
...
router.get('/api/students/:id', (req, res) => {
  ...
  db.query(
    `SELECT full_name, email, phone, institution, field_of_study, year_of_study FROM students WHERE student_id = ?`,
    [studentId],
    (err, results) => {
      if (err) return res.status(500).json({ message: 'Database error', error: err });
...
// ➕ Create Bursary
router.post('/api/bursaries', (req, res) => {
  const {
...
// 📥 Get All Bursaries
router.get('/api/bursaries', (req, res) => {
...
router.get('/api/bursaries/available', (req, res) => {
  db.query(
...
// ✏️ Update Bursary
router.put('/api/bursaries/:id', (req, res) => {
...
// ❌ Delete Bursary
router.delete('/api/bursaries/:id', (req, res) => {
  const id = req.params.id;
...
// 🔄 Withdraw Application (POST with body)
router.post('/api/applications/withdraw', (req, res) => {
  const { student_id, bursary_id } = req.body;
...
// 🗂️ Get Full Admin View of Applications
...
router.post('/api/status/update', (req, res) => {
  const {
...
// 🧾 Upload Document
router.post('/api/upload-document', upload.single('file'), (req, res) => {
...
router.get('/api/student/:studentId/documents', (req, res) => {
...
router.get('/api/student/applications/:studentId', (req, res) => {
  const studentId = req.params.studentId;
...
router.delete('/api/delete-document/:documentId', (req, res) => {
...
router.get('/api/applications/:applicationId/documents', (req, res) => {
...
router.get('/api/admins', (req, res) => {
  db.query(`SELECT admin_id, full_name, email FROM admins ORDER BY full_name ASC`, (err, results) => {
...
router.get('/api/students', (req, res) => {
  db.query(`SELECT student_id, full_name, email FROM students ORDER BY full_name ASC`, (err, results) => {
...
router.post('/api/conversations/initiate', (req, res) => {
  ...
  db.query(checkSQL, [student_id, admin_id], (checkErr, result) => {
    if (checkErr) return res.status(500).json({ message: 'Check failed', error: checkErr });
    ...
    const insertSQL = `INSERT INTO conversations (student_id, admin_id) VALUES (?, ?)`;

    db.query(insertSQL, [student_id, admin_id], (insertErr, insertResult) => {
      if (insertErr) return res.status(500).json({ message: 'Conversation creation failed', error: insertErr });
...
// ✅ Final Batch of Messaging Routes

// 📥 Get all admins a student has conversed with
router.get('/api/conversations/student/:studentId', (req, res) => {
...
router.get('/api/conversations/admin/:adminId', (req, res) => {
...
router.get('/api/messages/conversation/:conversationId', (req, res) => {
...
router.post('/api/messages/mark-read', (req, res) => {
...
router.get('/api/student/:studentId/applications', (req, res) => {
  const studentId = req.params.studentId;
